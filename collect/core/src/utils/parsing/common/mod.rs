...
#[inline]
pub fn split_eq_tuple<'a>(s : &'a str) -> Result<(&'a str, &'a str), Box<dyn Error>> {
...
#[inline]
pub fn split_colon_tuple<'a>(s : &'a str) -> Result<(&'a str, &'a str), Box<dyn Error>> {
    let v : Vec<&str> = s.split(":").collect();
    if v.len() != 2 {
        return utils_new_error_crate!(data, CantSplitTupleError, s);
    }
...
where T : Default, F : Fn(&'_ str)-> Result<(&'_ str, &'_ str), Box<dyn Error>>,
F2 : Fn(&'_ str, &'_ str, &mut T) -> Result<(), Box<dyn Error>>{
    ...
    for item in v {
        ...
        mapping_val_fn(key, value, &mut ret)?;
    }
