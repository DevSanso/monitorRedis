...
pub fn info_cpu_worker(link_key : i32, redis_conn : &'_ mut dbs::redis_pool::RedisRequester, pg_conn : &'_ mut dbs::pg_pool::PgConnecter) -> Result<(),Box<dyn Error>> {
    redis_conn.set_app_name("collect_info_cpu")?;
    ...
    pg_conn.execute(&pg_query,
         &[&link_key, &make_pg_numeric(c.cpu_sys), &make_pg_numeric(c.cpu_user), &make_pg_numeric(c.child_cpu_sys), &make_pg_numeric(c.child_cpu_user), &c.total_sec])?;
    Ok(())
}