...
pub struct RedisThreadExecutor {
    ...
    pub(super) redis_conn_flag : Vec<RedisConnCfg>,
    pub(super) single_conn_fn : SelectRedisConnFn,
    pub(super) worker_fn_list : HashMap<&'static str, (Duration, WorkerFn)>,
    pub(super) redis_pools : HashMap<RedisHashCode, (Arc<Mutex<RedisPool>>, RedisConnRunFlag, RedisLinkId)>,
...
impl RedisThreadExecutor {
    fn pop_redis_connect_list(h_ref : &mut HashMap<RedisHashCode, (Arc<Mutex<RedisPool>>, RedisConnRunFlag, RedisLinkId)>, new_list : &'_ [RedisConnCfg]) {
    ...
    fn push_redis_connect_list(h_ref : &mut HashMap<RedisHashCode, (Arc<Mutex<RedisPool>>, RedisConnRunFlag, RedisLinkId)>, new_list : &'_ [RedisConnCfg]) {
        let size = new_list.len();
        ...
        for new_p in push_list {
            h_ref.insert(new_p.1.clone(), (Arc::new(Mutex::new(new_p.0)), HashMap::new(), new_p.2));
        }
    }
