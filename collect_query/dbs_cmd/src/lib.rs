...
pub static COLLECT_COMMANDLINE_MAP: once_cell::sync::Lazy<HashMap<CollectCommand, &'_ str>> =
    once_cell::sync::Lazy::new(|| {
        ...
        collect_commandline_map_internal.insert(CollectCommand::CommonMem," INSERT INTO comm_mem (   server_id,   collect_time,   total_kb,   usage_kb,   free_kb  ) VALUES ( $1, now(), $2, $3, $4) ");
        ...
        collect_commandline_map_internal.insert(CollectCommand::CommonConfigAll," INSERT INTO comm_config_all(   server_id,   sync_time,   name,   value )  VALUES (   $1, now(), $2, $3 )  ON CONFLICT(server_id, name)   DO UPDATE SET   sync_time = now(),   value = $3 ");
        collect_commandline_map_internal.insert(CollectCommand::CommonPingUpdate," INSERT INTO comm_ping_status(   server_id,   sync_time,   status )  VALUES (   $1, now(), $2)  ON CONFLICT(server_id)   DO UPDATE   sync_time = now(),   status = $2");
...
pub static MANAGE_COMMANDLINE_MAP: once_cell::sync::Lazy<HashMap<ManageCommand, &'_ str>> =
    once_cell::sync::Lazy::new(|| {
        ...
        manage_commandline_map_internal.insert(ManageCommand::RedisConnInfo," SELECT redis_id, username, password, dbname, ip, port FROM redis_conn where redis_id = $1");
